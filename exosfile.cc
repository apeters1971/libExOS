...
int 
exosfile::stat(uint64_t& size, time_t& mtime)
{
  return 0;
}
...
exosfile::open(int flags)
{
  ...
  if (isRead()) {
    // fetch from metadata pool and fill attributes
    retc = open_md();
  } else if (isWrite()) {
    // fetch from metadata pool and fill attributes
    retc = open_md();
  } else if (isCreate()) {
    // create in metadata pool 
    retc = create_md();
  }
...
exosfile::write(const char* buffer, off_t offset, size_t len)
{
  ...
  for (auto it=objmap.begin(); it!=objmap.end(); ++it)
  {
    ...
    b.append(buffer+it->offset,it->len);
...
exosfile::aio_write(const char* buffer, off_t offset, size_t len)
{
  ...
  if (mSeqWrite && (offset == mWritePosition))
  {
	...
	ChunkWMap[mSeqWriteHandler->offset] = mSeqWriteHandler;
  ...
  for (auto writ=objmap.begin(); writ!=objmap.end(); ++writ)
  {
    ...
    dst->buffer.append(buffer+writ->offset,writ->len);

    ChunkWMap[(uint64_t) dst->offset] = dst;
...
exosfile::aio_flush()
{
  ...
  {
    it->second->completion->wait_for_safe_and_cb();
    retc |= it->second->completion->get_return_value();
    auto delit = it;
    ++it;
...
exosfile::aio_collect()
{
  ...
  for (auto it = ChunkWMap.begin(); it != ChunkWMap.end();)
  {
    if (it->second->completion->is_safe_and_cb())
    {
      retc |= it->second->completion->get_return_value();
      auto delit = it;
      ++it;
