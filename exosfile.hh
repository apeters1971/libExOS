...
public:
  ...
  exosfile() {mOpened = false;};
  void init(const std::string name, const std::string cgi);

  ~exosfile();

  int stat(uint64_t& size, time_t& mtime);
...
private:
  ...
  bool isCreate() const { return mFlags & O_CREAT; } 
  bool isWrite() const { return ((mFlags & O_WRONLY) || (mFlags & O_RDWR)); } 
  bool isRead() const { return !(isCreate() || isWrite()); }
  ...
  std::map<uint64_t, io_handler> ChunkWMap;
